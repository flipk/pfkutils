                             ...
                             uint16_t length_b)
{
	...
	uint8_t b[64];
        uint8_t *c = b;
	while (length_b >= 512){
		websocket_md5_nextBlock(state, block);
		length_b -= 512;
	...
	if(l+sizeof(uint64_t) >= 512/8){
		websocket_md5_nextBlock(state, b);
		state->counter--;
		memset(b, 0, 64-8);
	}
	*((uint64_t*)&c[64-sizeof(uint64_t)]) = (state->counter * 512) + length_b;
	websocket_md5_nextBlock(state, b);
}
...
{
	memcpy(dest, state->a, WEBSOCKET_MD5_HASH_BYTES);
}

void websocket_md5(websocket_md5_hash_t dest, const void* msg, uint32_t length_b)
{
	websocket_md5_ctx_t ctx;
	websocket_md5_init(&ctx);
	while(length_b>=WEBSOCKET_MD5_BLOCK_BITS){
		websocket_md5_nextBlock(&ctx, msg);
		msg = (uint8_t*)msg + WEBSOCKET_MD5_BLOCK_BYTES;
		length_b -= WEBSOCKET_MD5_BLOCK_BITS;
	}
	websocket_md5_lastBlock(&ctx, msg, length_b);
	websocket_md5_ctx2hash(dest, &ctx);
}
