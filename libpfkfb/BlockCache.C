...
{
    io = _io;
    pc = new PageCache( io, max_bytes / PageCache::PC_PAGE_SIZE );
...
BlockCache :: get( off_t offset, int size, bool for_write )
{
    ...
    starting_page = offset / PageCache::PC_PAGE_SIZE;
    ending_page = ending_offset / PageCache::PC_PAGE_SIZE;
    num_pages = ending_page - starting_page + 1;

    offset_in_starting_page = offset % PageCache::PC_PAGE_SIZE;
    offset_in_ending_page = ending_offset % PageCache::PC_PAGE_SIZE;
    ...
    for (pg=starting_page, i=0; i < num_pages; pg++,i++)
    {
        ...
        if (for_write && size >= PageCache::PC_PAGE_SIZE)
        {
            ...
            else if (pg == starting_page && offset_in_starting_page == 0)
            {
                ...
                for_write_pg = true;
                     ...
                     offset_in_ending_page == (PageCache::PC_PAGE_SIZE-1))
            {
        ...
        if (uptr && remaining > 0)
        {
            ...
            if (to_copy > (PageCache::PC_PAGE_SIZE - pg_offset))
                to_copy = PageCache::PC_PAGE_SIZE - pg_offset;
...
BlockCache :: release( BlockCacheBlock * _bcb, bool dirty )
{
    ...
    if (dirty && bcb->num_pages > 1)
    {
        uptr = bcb->ptr;
        remaining = bcb->size;
        pg_offset = bcb->offset % PageCache::PC_PAGE_SIZE;
    }
...
BlockCache :: truncate( off_t offset )
{
    ...
    int pages = (offset + PageCache::PC_PAGE_SIZE - 1) / PageCache::PC_PAGE_SIZE;
