...
/*
 * this is a memory poolset used by threads itself
 * for stacks and thread-control structures.
 */

#define NUMPOOLS 2
static int quantities[] = { MAX_THREADS, MAX_THREADS };
static int sizes[] = { sizeof(thread_t), STACK_SIZE };
static mpoolset *pools;
...
__inline static void        th_sw             (enum threadstate);
...
th_init(void)
{
	...
	pools = poolset_init(NUMPOOLS, quantities, sizes);
	numthreads = 0;
	free_later = NULL;
	sleepq = current = NULL;
	thread_ticks = 0;
...
th_loop(void)
{
	...
	/* 
	 * not really necessary, since we're destroying the 
	 * poolset anyway; but I'm a pedantic bastard.
	 */
	...
	char *name;
{
	...
	new = poolset_alloc(pools, sizeof(thread_t));
	if (new == NULL)
	{
	...
	new->stack = poolset_alloc(pools, STACK_SIZE);
	if (new->stack == NULL)
	{
		poolset_free(pools, new);
	...
	int tid;
{
	...
	else
	{
		...
		poolset_free(pools, target->stack);
	}

	free(target->name);
	threads[tid] = NULL;
	poolset_free(pools, target);
	numthreads--;
...
int
th_select(nrfds, rfds, nwfds, wfds, nofds, ofds, ticks)
	int nrfds, nwfds, nofds, *rfds, *wfds, *ofds;
	int ticks;
{
	int i;
	int ret;
