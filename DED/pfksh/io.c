...
{
	/* Avoid foo: foo[2]: ... */
	if (!fileline || !yysource || !yysource->file
	    || strcmp(yysource->file, kshname) != 0)
		shf_fprintf(shl_out, "%s: ", kshname + (*kshname == '-'));
	if (fileline && yysource && yysource->file != NULL) {
		shf_fprintf(shl_out, "%s[%d]: ", yysource->file,
			yysource->errline > 0 ? yysource->errline : yysource->line);
		yysource->errline = 0;
	}
}
	...
	struct temp **tlist;
{
	...
	while (1) {
		...
		if (errno != EINTR && errno != EEXIST && errno != EISDIR)
