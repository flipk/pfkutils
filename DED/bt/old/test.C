...
main( int argc, char ** argv )
{
    ...
    try {
        frn    = new FileRecordNumber( FRNARGS );
        frnmap = new FileRecordNumberMap( frn );
        fbn    = new FileBlockNumber( frnmap, FBNARGS );
    ...
    catch ( FileRecordNumber::constructor_failed r ) {
// ? why does this work, r is an aggregate
        printf( "frn open failure %d\n", r );
        return 1;
    }
...
#define TEST4VERBOSE 0
    ...
    for ( i = 0; i < NUMROUNDS; i++ )
    {
        ...
        if ( lastt != now )
        {
            ...
            lines++;
            printf( "%4d %7d (%5d) %7d %4d %4d %4d %7d\n",
                    now - startt,
                    i, delta_i, numindb,
                    put, got, del,
                    bt->flush()
