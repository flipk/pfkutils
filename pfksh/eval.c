...
#define stat_check()	(stat_done ? stat_done : \
			    (stat_done = stat(Xstring(*xs), &statb) < 0 \
				? -1 : 1))
			struct stat lstatb, statb;
            int cc;
			int stat_done = 0;	 /* -1: failed, 1 ok */

            cc = lstat(Xstring(*xs), &lstatb);
			if (cc < 0)
            {
                fprintf(stderr, "can't stat %s: %s\n",
                        Xstring(*xs), strerror(errno));
				return;
            }
			 ...
			 */
			if ((check & GF_EXCHECK) && xp > Xstring(*xs)
...
# define KLUDGE_VAL	0
		XPput(*wp, str_nsave(Xstring(*xs), Xlength(*xs, xp)
			+ KLUDGE_VAL, ATEMP));
		return;
	}
	...
	if (!has_globbing(sp, se)) {
		...
		*xp = '\0';
		...
		dirp = opendir(prefix_len ? Xstring(*xs) : ".");
		if (dirp == NULL)
			goto Nodir;
	...
	int isassign;
{
	...
	*tp = '\0';
	r = (p[0] == EOS || p[0] == CHAR || p[0] == CSUBST) ? tilde(Xstring(ts)) : (char *) 0;
	Xfree(ts, tp);
	if (r) {
		while (*r) {
			Xcheck(*dsp, dp);
