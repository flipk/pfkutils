	...
	char	**wp;
{
...
#ifdef S_ISLNK
		if (physical)
			rval = chdir(try = Xstring(xs) + phys_path);
		else
#endif /* S_ISLNK */
		{
			simplify_path(Xstring(xs));
			rval = chdir(try = Xstring(xs));
		}
	...
	if (!ISABSPATH(Xstring(xs))) {
		pwd = (char *) 0;
	} else
#ifdef S_ISLNK
	if (!physical || !(pwd = get_phys_path(Xstring(xs))))
#endif /* S_ISLNK */
		pwd = Xstring(xs);
...
#define PO_FSLASH	BIT(5)  /* swap slash for backslash (for os2 ) */
	...
	if (flags & PO_HIST) {
		Xput(xs, xp, '\0');
		yysource->line++;
		histsave(yysource->line, Xstring(xs), 1);
		Xfree(xs, xp);
	} else {
