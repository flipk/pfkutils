	...
	size_t len;
{
	...
	*xcp = 0;
	...
	if (x_nextcmd >= 0) {
		int off = yysource->line - x_nextcmd;
	...
	int c;
{
	if (x_arg_defaulted)
		...
		x_load_hist(histptr + x_arg - yysource->line);
	return KSTD;
}
...
x_nl_next_com(c)
	int	c;
{
	x_nextcmd = yysource->line - (histptr - x_histp) + 1;
	return (x_newline(c));
}
