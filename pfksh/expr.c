...
struct expr_state {
	...
	enum token_t  tok;		/* token from token() */
...
static struct tbl *do_ppmm  ARGS((Expr_state *es, enum token_t op,
				  struct tbl *vasn, bool_t is_prefix));
static void	   assign_check ARGS((Expr_state *es, enum token_t op,
				      struct tbl *vasn));
