...
static void     motion_detect   __P((int, int));
...
"-z is exclusive with everything else.\n"
"\n", progname, progname);
...
qcam_main( int argc, char ** argv )
{
	...
	while ((ch = getopt(argc, argv, "qm:os:pM:d:tx123z:")) != -1)
		...
		case 'z':
			...
			optind++;
			SET(MOTION_DET);
			printf("pixel threshold = %d, frame threshold = %d\n",
	...
	if (ISSET(MOTION_DET))
	{
		motion_detect(pixel_threshold, frame_threshold);
		return 0;
	}
