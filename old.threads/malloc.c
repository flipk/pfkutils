...
void malloclock( int );
#   define THREAD_LOCK()     malloclock(1)
#   define THREAD_UNLOCK()   malloclock(0)
...
#define sig1_val 0xef /* 32 bytes before link info   */
#define sig2_val 0xef /* 32 bytes after link info    */
#define sig3_val 0xef /* 64 bytes after malloc block */
#define sig1_size 32
#define sig2_size 32
#define sig3_size 64
#define footer(hdr) (((uchar*)hdr)+hdr->size-sig3_size)
#define hdr_size (sizeof(mallochdr)+sig3_size)
#else
#define hdr_size sizeof(mallochdr)
#endif

#define payload(hdr) ((void*)(hdr+1))
#define hdr(ptr) (((mallochdr*)ptr)-1)
#define uchar unsigned char

typedef struct _mallochdr {
#ifdef MALLOC_REDZONES
    uchar sig1[ sig1_size ];
#endif
...
static mallochdr * mallocs_h = NULL;
static mallochdr * mallocs_t = NULL;
...
{
#ifdef MALLOC_REDZONES
    memset( h->sig1,   sig1_val, sig1_size );
    memset( h->sig2,   sig2_val, sig2_size );
    memset( footer(h), sig3_val, sig3_size );
#endif
    h->next = mallocs_h;
    h->prev = NULL;
    if ( mallocs_h )
...
{ \
    char s[100]; \
    strcpy( s, "error in malloc, signature X missing!\n\r" ); \
    s[27] = '0' + x; \
    write(2,s,strlen(s)); \
}
...
m_del( mallochdr * h )
{
    ...
    for ( i = 0; i < sig1_size; i++ )
        if ( h->sig1[i] != sig1_val )
            BAIL(1);
    for ( i = 0; i < sig2_size; i++ )
        if ( h->sig2[i] != sig2_val )
            BAIL(2);
    p = footer(h);
    for ( i = 0; i < sig3_size; i++ )
        if ( p[i] != sig3_val )
            BAIL(3);
#endif
    if ( h->next )
