...
#define MAXSTARS 100        /* Number of stars issued from a shell */
#define MINSTARS 50
...
/* All this to stop too many rockets going offscreen: */
    ...
    while (multi--)
    {
        ...
        if (!mono && (npix = Scr[screen].npixels) > 2)
        {
            ...
            {
                pix = random() % (npix + 1);
                rp->color1 = WhitePixel (dsp, screen);
                rp->color2 = Scr[screen].pixels[pix];
            }
            else
            {
                rp->color1 = Scr[screen].pixels[pix];
                rp->color2 = Scr[screen].pixels[(pix + (npix / 2)) % npix];
            }
        }
        else
        {
            rp->color1 = rp->color2 = WhitePixel(dsp, screen);
        }
