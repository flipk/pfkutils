 ...
 *
 * Revision History:
 * 22-Jan-93: Added spline mode and points option.
...
static LockStruct LockProcs[] = {
    ...
    {"rotor", initrotor, drawrotor, 50000, 4, 0.4, "Tom's Roto-Rooter"},
    {"pyro", initpyro, drawpyro, 5000, 40, 1.0, "Fireworks"},
    {"flame", initflame, drawflame, 75000, 20, 1.0, "Cosmic Flame Fractals"},
...
#define DEF_PT          "5"     /* number of spline points */
#define DEF_DELAY   "50000"/* microseconds between batches */
...
static XrmOptionDescRec modeTable[] = {
    ...
    {"-points", ".points", XrmoptionSepArg, (caddr_t) NULL},
};
...
static OptionStruct opDesc[] = {
    ...
    {"-points num", "number of spline points"},
    ...
    )
{
    ...
    case t_String:
    {
        ...
        if (s == (char *) NULL)
            errx( 1, "GetResource - couldn't allocate memory" );
...
//        extern char *strchr();
        ...
        if (colon == NULL)
            errx( 1, "Malformed -display argument, \"%s\"\n", display);
        ...
        if (!remote && n
            ...
            if (gethostname(hostname, MAXHOSTNAMELEN))
                errx( 1, "Can't get local hostname.\n");

            if (!(host = gethostbyname(hostname)))
                errx( 1, "Can't get hostbyname.\n");
...
void initrandommode(void)
{
    ...
    delay = LockProcs[current_mode].def_delay;
    batchcount = LockProcs[current_mode].def_batchcount;
    saturation = LockProcs[current_mode].def_saturation;

    current_mode++;
    current_mode %= NUMPROCS;
}
